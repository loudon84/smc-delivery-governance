...
Product version is `6.0.0-alpha.4`. Overlay files live under `.ges/capabilities/` and are created only by `add`/`remove`. `ges init` still prints the Alpha.3 recommendation and must not create overlay files. Doctor and check ignore overlay for overall. See [[capability-resolver]] and [[capability-governance-tests]].
...
Detached observation of `list` is not a READY claim.

If the Golden consumer is missing, dirty, or unrun, the runner prints `GOLDEN_CAPABILITY_GOVERNANCE_BLOCKED`. Synthetic suites must not fail because Golden is BLOCKED. See [[ges/acceptance/run_golden_capability_governance.py#main]].
