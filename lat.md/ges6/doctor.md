...
Alpha.3 adds ephemeral `capabilities[]` and `warnings[]` for RTK verify. Missing recommended RTK is WARNING and does not BLOCK overall. A closed set with no `superpowers.*` IDs is not a Superpowers FAIL. See [[capability-resolver]].
