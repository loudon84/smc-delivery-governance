...
REQ=["Change ID","Owner","Contract","Data/Transaction","Auth","Idempotency/Concurrency","Failure Semantics","Observability"]
def validate(p):
 required=list(REQ)
 # Explicit justified N/A is allowed.
 return validate_table(p,"Backend Design Intent",required,"BACKEND_PREPLAN")
val=validate
def main():
