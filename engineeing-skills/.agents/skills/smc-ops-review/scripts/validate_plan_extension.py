...
REQ=["Change ID","Deployment Impact","Compatibility","Environment/Config","Health","Migration Order","Rollback","Verification"]
def validate(p):
 required=list(REQ)
 # Explicit justified N/A is allowed.
 return validate_table(p,"Ops Quality Ledger",required,"OPS_REVIEW")
val=validate
def main():
