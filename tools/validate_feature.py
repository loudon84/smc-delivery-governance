...
from artifact_verify import verify_artifact_ref
from governance_lib import (
    ROOT, contract_catalog, load_feature, load_work_packages, load_yaml,
    repository_catalog, validate_jsonschema,
)
...
def main():
    ...
    fdir,feature=load_feature(args.feature);errors=[]
    ...
    if not errors and not args.offline:
        try:
            errors += [f"source_prd {e}" for e in verify_artifact_ref(feature["source_prd"],token=args.token)]
        except RuntimeError as exc:
            errors.append(f"source_prd semantic verification failed: {exc}")
    ...
    for c in feature.get("contracts",[]) or []:
        cid=c["contract_id"]
        if "current_state" in c:errors.append(f"{cid}: Feature must not cache current_state")
