...
from governance_lib import ROOT, load_feature, load_work_packages, load_yaml, resolve_contract, state_at_least, CONTRACT_ORDER, WP_ORDER
...
def main():
    ...
    for req in scenario.get("requires", {}).get("contracts", []):
        consumer_repo = next((wp["repository_id"] for wp in wps.values() if wp.get("role") == "consumer"), None)
        version = req.get("required_version")
        current = resolve_contract(req["contract_id"], version, consumer_repo)
    ...
    if blockers:
        provider_block = any(
            t == "work_package" and (wps.get(i) or {}).get("role") == "provider" for t, i, _, _ in blockers
        )
        consumer_block = any(
            t == "work_package" and (wps.get(i) or {}).get("role") == "consumer" for t, i, _, _ in blockers
        )
