...
from governance_lib import ROOT, dump_yaml, repository_catalog
from upgrade_source_prd_artifact import build_source_prd_artifact
...
def main():
    ...
    ap.add_argument('--source-prd-id',required=True)
    ap.add_argument('--source-prd-repo',required=True)
    ap.add_argument('--source-prd-path',required=True)
    ap.add_argument('--source-revision',required=True)
    ap.add_argument('--source-prd-ref',help='branch/tag/commit used to resolve ArtifactRef identity')
    ap.add_argument('--source-prd-commit')
    ap.add_argument('--source-prd-blob-sha')
    ap.add_argument('--source-prd-sha256')
    ap.add_argument('--token')
    ap.add_argument('--feature-owner',required=True)
    ap.add_argument('--integration-owner',required=True)
    ...
    source_prd=build_source_prd_artifact(
        repository_id=args.source_prd_repo,
        path=args.source_prd_path,
        artifact_id=args.source_prd_id,
        status='APPROVED',
        source_revision=args.source_revision,
        ref=args.source_prd_ref,
        commit=args.source_prd_commit,
        blob_sha=args.source_prd_blob_sha,
        sha256=args.source_prd_sha256,
        token=args.token,
    )
    feature={
    ...
    dump_yaml(fdir/'traceability.yaml',{
    ...
    (fdir/'architecture.md').write_text(
        f"# {args.feature_id} Architecture\n\nSource PRD ArtifactRef `{source_prd['artifact_id']}` @ `{args.source_revision}` (`{source_prd['commit']}`).\n\nDefine cross-repo ownership and interface boundaries here; do not write repository implementation details.\n",
