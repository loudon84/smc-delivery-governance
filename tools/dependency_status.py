...
from governance_lib import load_feature, load_work_packages, load_yaml, resolve_contract, state_at_least, CONTRACT_ORDER, WP_ORDER
...
def main():
    ...
    for dep in deps_doc.get("dependencies", []):
        ...
        if target["type"] == "contract":
            ...
            current = resolve_contract(target["id"], target.get("version"), consumer_repo)
        ...
        print(
            f"{'PASS' if ok else 'BLOCK':5} {dep['id']:8} {dep['from']} -> {target['type']}:{target['id']} "
            f"required={dep['required_state']} current={current}"
        )
        blocked |= not ok
    if blocked:
        raise SystemExit(2)
