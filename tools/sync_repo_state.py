...
def consumer_receipt_ok(receipt: dict) -> bool:
    delivery = receipt.get("delivery") or {}
    if receipt.get("status") not in {"VERIFIED", "DONE"}:
        return True
    return bool(delivery.get("commits") and (receipt.get("acceptance") or {}).get("status") == "PASS")
...
def _refresh_traceability(fdir: Path, wps: dict) -> None:
    ...
    for wid, wp in wps.items():
        ...
        item = by_id.get(wid) or {
            "work_package_id": wid,
            "repository_id": wp["repository_id"],
        }
        item["stage_prds"] = delivery.get("stage_prds") or []
        item["issues"] = delivery.get("issues") or []
        item["bugs"] = delivery.get("bugs") or []
        item["plans"] = delivery.get("plans") or []
        item["commits"] = delivery.get("commits") or []
        item["verification"] = delivery.get("verification_reports") or []
        by_id[wid] = item
...
def main():
    ...
    ap.add_argument(
        "--local-receipt-dir",
        help="Read receipts from local directory named <repository_id>.yaml (for tests/samples)",
    )
    args = ap.parse_args()
    ...
    for wid, wp in wps.items():
        ...
        if not repo:
            print(f"{wid}: DIVERGED unknown repository")
        ...
        try:
            ...
            print(f"{wid}: DIVERGED invalid yaml: {exc}")
            expected_non_ready = True
            continue

        errors = validate_jsonschema(receipt, ROOT / "schemas/delivery-receipt.schema.json")
        if errors:
            print(f"{wid}: DIVERGED invalid receipt: {errors[0]}")
            expected_non_ready = True
            continue

        if (
            receipt["feature_id"] != feature["feature_id"]
            or receipt["work_package_id"] != wid
            or receipt["repository_id"] != wp["repository_id"]
        ):
            print(f"{wid}: DIVERGED identity mismatch")
            expected_non_ready = True
            continue
        ...
        for cid, ver in expected.items():
            if ver and actual.get(cid) != ver:
                sync_state = "STALE_CONTRACT"

        claimed = receipt["status"]
        if claimed in {"VERIFIED", "DONE"}:
            if wp.get("role") == "provider" and wp.get("contract_outputs"):
                if not provider_receipt_ok(receipt, wp):
                    sync_state = "DIVERGED"
            elif not consumer_receipt_ok(receipt):
                sync_state = "DIVERGED"

        if args.discover_github and not args.local_receipt_dir:
            try:
                issues, bugs, prs = discover_github(repo["name"], wid, args.token)
                receipt["delivery"]["issues"] = issues
                receipt["delivery"]["bugs"] = bugs
                receipt["delivery"]["pull_requests"] = prs
            except RuntimeError as exc:
                message = str(exc)
                if "GitHub API 401" in message or "GitHub API 403" in message:
                    print(f"{wid}: SYSTEM_ERROR GitHub auth failed")
                    raise SystemExit(EXIT_SYSTEM_ERROR) from exc
        ...
        if args.apply:
            ledger = {
                ...
                "receipt_path": resolved_path or receipt_path,
                "status": claimed,
    ...
    if args.apply:
        _refresh_traceability(fdir, wps)
